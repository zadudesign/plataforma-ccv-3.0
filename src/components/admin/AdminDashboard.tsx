...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const areaSeleccionadaObj = areas.find(
    a => a.nombre.toLowerCase() === filtroAreaAsignaciones.toLowerCase() || a.id === filtroAreaAsignaciones
  );
  const nivelSeleccionado = areaSeleccionadaObj ? areaSeleccionadaObj.nivel : null;
  const esFiltroTodas = filtroAreaAsignaciones === 'todas';

  // Visibilidad de secciones según nivel jerárquico
  // Nivel 6: ADMIN (Muestra todo)
  // Nivel 5: CMU / Áreas Operativas / Producción (Muestra Proyectos)
  // Nivel 4: DEPARTAMENTO / Áreas de Innovación (Muestra Proyectos)
  // Nivel 3: FACULTAD (Muestra Facultades, Programas y Cursos)
  // Nivel 2: PROGRAMA (Muestra Programas y Cursos)
  // Nivel 1: CURSO (Muestra Cursos)
  const mostrarSeccionFacultades = esFiltroTodas || nivelSeleccionado === 6 || nivelSeleccionado === 3 || filtroAreaAsignaciones.toUpperCase() === 'FACULTAD';
  const mostrarSeccionProgramas = esFiltroTodas || nivelSeleccionado === 6 || nivelSeleccionado === 3 || nivelSeleccionado === 2 || filtroAreaAsignaciones.toUpperCase() === 'FACULTAD' || filtroAreaAsignaciones.toUpperCase() === 'PROGRAMA';
  const mostrarSeccionCursos = esFiltroTodas || nivelSeleccionado === 6 || nivelSeleccionado === 3 || nivelSeleccionado === 2 || nivelSeleccionado === 1 || filtroAreaAsignaciones.toUpperCase() === 'FACULTAD' || filtroAreaAsignaciones.toUpperCase() === 'PROGRAMA' || filtroAreaAsignaciones.toUpperCase() === 'CURSO';
  const mostrarSeccionProyectos = esFiltroTodas || nivelSeleccionado === 6 || nivelSeleccionado === 5 || nivelSeleccionado === 4 || (!mostrarSeccionFacultades && !mostrarSeccionProgramas && !mostrarSeccionCursos);

  // Áreas aplicables para la sección de proyectos
  const areasFiltradasParaProyectos = areas.filter(area => {
    if (esFiltroTodas || nivelSeleccionado === 6) return true;
    if (area.nombre.toLowerCase() === filtroAreaAsignaciones.toLowerCase() || area.id === filtroAreaAsignaciones) {
      return true;
    }
    if (areaSeleccionadaObj && (area.parent_id === areaSeleccionadaObj.id || area.area_padre_nombre?.toLowerCase() === areaSeleccionadaObj.nombre.toLowerCase())) {
      return true;
    }
    return false;
  });
    ...
    <div className="space-y-6 animate-fadeIn font-sans">
      ...
      {pestana === 'asignaciones' && (
        <div className="space-y-6">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {filtroAreaAsignaciones !== 'todas' && (
                  <button
                    ...
                    className="px-2.5 py-1 bg-white hover:bg-stone-100 text-coral-600 rounded-lg text-xs font-bold border border-stone-200 transition-colors flex items-center gap-1 shadow-2xs"
                  >
                    <RotateCcw className="w-3 h-3" /> Limpiar Filtro
                  </button>
                )}
              </div>
            </div>
              ...
              {areas.map(a => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer ${isSelected
                        ? 'bg-sage-700 text-white shadow-xs ring-2 ring-sage-500'
                        : 'bg-white text-charcoal-700 hover:bg-cream-100 border border-stone-200 hover:border-sage-300'
                      }`}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-amber-400' : 'bg-sage-500'}`}></span>
                    <span>{a.nombre}</span>
                    <span className={`text-[9px] font-semibold ${isSelected ? 'text-sage-100' : 'opacity-75'}`}>(N{a.nivel})</span>
                  </button>
                );
              })}
            </div>

            {/* Banner de Estado del Filtro Jerárquico Activo */}
            {filtroAreaAsignaciones !== 'todas' && (
              <div className="mt-2 p-3 bg-sage-50/90 rounded-2xl border border-sage-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs animate-fadeIn">
                <div className="flex items-center gap-2.5">
                  <span className="px-2 py-0.5 rounded-lg bg-sage-700 text-white font-black text-[10px] shadow-2xs">
                    Nivel {areaSeleccionadaObj?.nivel ?? '—'}
                  </span>
                  <div>
                    <p className="font-extrabold text-charcoal-900 leading-tight">
                      Jerarquía activa: <span className="text-sage-800 underline decoration-sage-400">{areaSeleccionadaObj?.nombre || filtroAreaAsignaciones}</span>
                    </p>
                    <p className="text-[11px] text-charcoal-500 mt-0.5">
                      {mostrarSeccionFacultades && '✓ Facultades '}
                      {mostrarSeccionProgramas && '✓ Programas '}
                      {mostrarSeccionCursos && '✓ Cursos '}
                      {mostrarSeccionProyectos && '✓ Proyectos vinculados'}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setFiltroAreaAsignaciones('todas')}
                  className="px-3 py-1 bg-white hover:bg-stone-50 text-charcoal-700 hover:text-charcoal-900 rounded-xl text-xs font-bold border border-stone-200 shadow-2xs transition-all shrink-0 flex items-center gap-1.5"
                >
                  <RotateCcw className="w-3 h-3 text-sage-600" /> Restablecer a Todas
                </button>
              </div>
            )}
          </div>
          ...
          {/* Section 4: Registro de Proyectos (Clasificados internamente por Área Responsable) */}
            ...
            <div className="ccv-card p-6 space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-stone-100">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-extrabold text-charcoal-900">4. Registro de Proyectos</h3>
                      <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 border border-purple-200">
                        Clasificado por Área Responsable
                      </span>
                    </div>
                    <p className="text-xs text-charcoal-500 mt-0.5">
                      Iniciativas y proyectos especiales organizados por cada unidad o área responsable.
                    </p>
                  </div>
                </div>
                <button
              ...
              {/* Proyectos agrupados por Área filtrada */}
              <div className="space-y-4">
                {areasFiltradasParaProyectos.map(area => {
                  ...
                  if (proyectosDeEstaArea.length === 0 && !esFiltroTodas && areasFiltradasParaProyectos.length > 1) return null;
                ...
                {(esFiltroTodas || nivelSeleccionado === 6) && proyectos.filter(p => !areas.some(a => a.id === p.area_id || a.nombre === p.area_id)).length > 0 && (
                  <div className="p-4 bg-stone-50 rounded-3xl border border-stone-200 space-y-3">
                    <h4 className="font-bold text-charcoal-700 text-xs uppercase tracking-wider flex items-center gap-1.5">
                      <FolderKanban className="w-3.5 h-3.5" /> Proyectos Generales
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
